<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Jadwal Semester Ganjil</h6>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <?php if (!empty($jadwal) && !empty($jam_pelajaran)): ?>
                <table class="table table-bordered table-striped">
                    ...
                    <tbody>
                        <?php foreach ($jam_pelajaran as $jam_id => $jam_text): 
                            $jam_key = trim($jam_text); // pastikan key sesuai
                        ?>
                        <tr>
                            <td><strong><?= $jam_text ?></strong></td>
                            <td><?= $jadwal[$jam_key]['Senin']['mapel'] ?? '-' ?></td>
                            <td><?= $jadwal[$jam_key]['Selasa']['mapel'] ?? '-' ?></td>
                            <td><?= $jadwal[$jam_key]['Rabu']['mapel'] ?? '-' ?></td>
                            <td><?= $jadwal[$jam_key]['Kamis']['mapel'] ?? '-' ?></td>
                            <td><?= $jadwal[$jam_key]['Jumat']['mapel'] ?? '-' ?></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
                <?php else: ?>
                    <div class="alert alert-info mt-3">Jadwal belum tersedia untuk semester Ganjil.</div>
                <?php endif; ?>
            </div>
        </div>
    </div>
...
<div class="text-center mt-4 mb-4">
    <div class="school-park-anim-medium">
        <!-- SCALE 0.80 -->
        <svg width="350" height="270" viewBox="0 0 420 350" xmlns="http://www.w3.org/2000/svg" 
             style="transform: scale(0.80); transform-origin: top center;">
            ...
            <!-- BURUNG -->
            <path id="birdPath" d="M120 140 C 260 20, 330 200, 180 270 C 60 220, 60 90, 120 140" fill="none"/>
            <g class="bird">
...
<style>
.school-park-anim-medium {
    animation: floaty 3s infinite ease-in-out;
}

/* Semua animasi tetap sama */
@keyframes floaty { 0% {transform:translateY(0);} 50% {transform:translateY(-8px);} 100% {transform:translateY(0);} }
.rainbow { opacity:0.75; animation: rainbowFloat 6s infinite ease-in-out; }
@keyframes rainbowFloat { 0%{transform:translateY(0);} 50%{transform:translateY(-6px);} 100%{transform:translateY(0);} }

.grass { animation: grassWave 4s infinite ease-in-out; }
@keyframes grassWave { 0%{transform:skewX(0deg);} 50%{transform:skewX(3deg);} 100%{transform:skewX(0deg);} }

.cloud { animation: cloudMove 14s infinite linear; }
@keyframes cloudMove { 0%{transform:translateX(-30px);} 100%{transform:translateX(40px);} }

.treeleaf { animation: leafWave 2.5s infinite ease-in-out alternate; }
@keyframes leafWave { 0%{transform:rotate(0);} 100%{transform:rotate(5deg);} }
...
.eye1a,.eye1b,.eye2a,.eye2b { animation: blink 3s infinite; }
@keyframes blink { 
    0%,90%,100% { transform: scaleY(1); }
    95% { transform: scaleY(0.1); }
}

.book-glow { animation: bookGlow 2s infinite ease-in-out; }
@keyframes bookGlow {
...
.flower circle { animation: flowerBloom 3s infinite ease-in-out; }
